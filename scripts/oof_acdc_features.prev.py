...
from utils.ef import ef_from_cavity  # if you use it elsewhere
# We’ll compute volumes via utils.ef.volume_ml when spacing is available.
...
class UNet3D(nn.Module):
    """Matches seg_cv.py: anisotropic upsampling + 4-class head when multiclass."""
    def __init__(self, in_ch=1, out_ch=4, feat=(16,32,64,128)):
        ...
        # anisotropic upsamples (1,2,2), like in seg_cv.py
    ...
    def _adaptive_pool(x):
        k = (1,2,2)
        return nn.functional.max_pool3d(x, kernel_size=k, stride=k)
...
def infer_fold(model, loader, multiclass=True, device=DEVICE):
    ...
    for batch in loader:
        ...
        lg = model(x)
        if multiclass:
            pr = torch.argmax(lg, dim=1)  # (B,Z,Y,X) in {0,1,2,3}
        else:
...
def main():
    ...
    for f, val_idx in enumerate(folds):
        ...
        if isinstance(state, dict) and 'state_dict' in state:  # tolerate wrappers
        ...
        for i, idx in enumerate(val_idx):
            ...
            pr = preds[i]
            # LV is class 3 under {0:bg,1:RV,2:MYO,3:LV}
